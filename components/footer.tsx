...
export function Footer() {
  ...
  return (
    <footer className="relative">
      {/* Main CTA */}
      <motion.a
        href="mailto:hmza.hb82@gmail.com"
        data-cursor-hover
      ...
      <div className="px-8 md:px-12 py-8 border-t border-white/10">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-8">
            {[
              { name: "LinkedIn", url: "https://linkedin.com/in/hmza-hb" },
              { name: "GitHub", url: "https://github.com/hmza-hb" },
              { name: "Instagram", url: "https://instagram.com/hamza.hafeez82" },
            ].map((link) => (
              <a
                key={link.name}
                href={link.url}
          ...
          {/* Copyright */}
          <p className="font-mono text-xs tracking-widest text-muted-foreground">© {new Date().getFullYear()}. Hamza Hafeez</p>
        </div>
      </div>
    </footer>
  )
